...
class MarioCore:
        ...
        for (r, c, trect, tt) in self._tile_rects_near(p.gObj):
            ...
            if ox < oy:
                ...
                if prect.centery < trect.centery:
                    ...
                    p.vy = max(0.0, p.vy)
                    if tt == TILE_QBLOCK: self._hit_qblock(c, r)
            prect = p.gObj.get_rect()
    ...
    def _check_termination(self) -> bool:
        ...
        if tile == TILE_GOAL:
            self.score += 1000; self.alive = False; self.reached_goal = True
            self.complete_level()
            return True
        if tile == TILE_SPIKE:
            self.alive = False; self.game_over = True; self.reached_goal = False
    ...
    def _player_obs(self) -> List[float]:
        p = self.player
        return [
            p.gObj.x / self.level_w,
            p.gObj.y / self.level_h,
    ...
    def _draw_ui(self, surface: pygame.Surface):
        ...
        ts = font.render(
            f"Step:{self.frame}  X:{int(p.gObj.x)}  Vx:{abs(p.vx):.2f}  "
            f"Score:{self.score}  Coins:{self.coins_total}  {status}",
            True, COLOR_WHITE
        )
        x = 5; y = 5
