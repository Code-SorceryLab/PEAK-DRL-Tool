...
class SonicCore(gymnasium.Env):
    ...
    def DEBUG_PANEL_X(self):
        return self.WIDTH if self.render_mode == "human" else 0

    @property
    def TOTAL_WIDTH(self):
        return self.WIDTH + DEBUG_PANEL_WIDTH if self.render_mode == "human" else self.WIDTH

    def __init__(self, render_mode: str = "none", **kwargs):
        self.render_mode = render_mode
        self.config_manager = ConfigManager("game_config.yaml")
        sonic_root = self.config_manager.yaml_data.get("sonic", {})
        if isinstance(sonic_root, dict) and sonic_root:
            self.config_manager.yaml_data = sonic_root
        else:
            self.config_manager = ConfigManager("sonic_config.yaml")
        self.raw_config = self.config_manager.yaml_data or {}
        self.WIDTH = int(self.raw_config.get("screen_width", SCREEN_WIDTH))
        self.HEIGHT = int(self.raw_config.get("screen_height", SCREEN_HEIGHT))
        self.FPS = int(self.raw_config.get("fps", 60))

        if self.render_mode == "human":
            pygame.init()
            pygame.display.set_caption("Sonic NES Clone — Green Hill Zone")
            self._surf = pygame.display.set_mode((self.WIDTH + DEBUG_PANEL_WIDTH, self.HEIGHT))
        else:
            os.environ.setdefault("SDL_VIDEODRIVER", "dummy")
            pygame.init()
            self._surf = pygame.Surface((self.WIDTH, self.HEIGHT))

        self.loader = LevelLoader(tile_size=int(self.raw_config.get("tile_size", TILE_SIZE)))
        ...
        self._debug_overlay_error_logged = False
        ...
        self.speed_mult = float(kwargs.pop("speed_mult", 1.0))
        self.physics_manager.speed_mult = self.speed_mult
        max_steps_cfg = kwargs.pop("max_steps", self.raw_config.get("episode", {}).get("max_steps", self.raw_config.get("max_steps", 3000)))
        ...
        self.max_lives = int((self.raw_config.get("defaults", {}) or {}).get("player", {}).get("lives", 3))
    ...
    def _rebuild_spatial_hashes(self):
        self.physics_manager.rebuild_dynamic_hashes(
            self.level_data,
            self._cached_spikes,
            rings=self.rings,
            lost_rings=self.lost_rings,
    ...
    def complete_level(self):
        ...
        if self.locked_level:
            self._pending_next_level_index = self.current_index_world
        else:
    ...
    def get_obs_value_labels(self):
        return (
            [
                "Px", "Py", "Vx", "Vy", "Grnd", "Ball", "Rings",
                "Invinc", "FaceR", "DashQ", "InvTmr", "Coyote", "Pad",
            ],
            [
    ...
    def _check_obs_sanity(self, obs):
        ...
        if self._obs_check_counter % self._obs_check_interval == 0 and (grids := obs.get("grids")) is not None:
            for i, name in enumerate(["solid", "collectible", "hazard", "dijkstra"]):
                ...
                self._obs_stats[f"grid_{name}_min"] = float(grids[i].min())
                self._obs_stats[f"grid_{name}_max"] = float(grids[i].max())
            scalars = obs.get("scalars")
            if scalars is not None:
                self._obs_stats["scalar_mean"] = float(np.mean(scalars))
                self._obs_stats["scalar_std"] = float(np.std(scalars))
                self._obs_stats["scalar_min"] = float(np.min(scalars))
                self._obs_stats["scalar_max"] = float(np.max(scalars))

    def _info(self) -> Dict:
        p = self.player
        ts = float(TILE_SIZE)
        event = "WIN" if self.reached_goal else ("DIED" if not self.alive else "")
    ...
    def get_jump_arc_debug_state(self) -> dict:
        ...
        if preview_jump:
            preview_vx = p.vx if abs(p.vx) > 20.0 else (
                float(getattr(ctx, "MAX_RUN_SPEED", 380.0)) * (0.55 if p.facing_right else -0.55)
            )
            preview_vy = float(getattr(ctx, "JUMP_VEL_MAX", -700.0))
        else:
            preview_vx = p.vx
            preview_vy = p.vy
        ...
        return {
            ...
            "color": (255, 220, 90) if preview_jump else (80, 190, 255),
        }

    def get_debug_hud_strip_items(self):
        player = self.player
        lives = max(0, int(getattr(self, "lives", 0)))
        score = int(getattr(self, "score", 0))
        rings = int(getattr(player, "rings", 0)) if player else 0
        timer = int(getattr(self, "timer", 0))

        lives_col = (255, 85, 75) if lives <= 1 else (210, 215, 228)
        time_col = (255, 85, 75) if timer < 60 else (210, 215, 228)
        ring_col = (255, 50, 50) if rings == 0 and self.frame % 30 < 15 else (220, 190, 60)

        if player:
            if getattr(player, "star_timer", 0.0) > 0.0:
                state = "STAR"
                state_col = (255, 195, 60)
            elif getattr(player, "powered_up", False):
                state = "SUPER"
                state_col = (255, 195, 60)
            else:
                state = getattr(getattr(player, "state", None), "name", "IDLE")
                state_col = (80, 225, 115)
        else:
            state = "—"
            state_col = (210, 215, 228)

        return [
            ("LIVES", str(lives), lives_col),
            ("SCORE", str(score), (210, 215, 228)),
            ("RINGS", str(rings), ring_col),
            ("STATE", state, state_col),
    ...
    def render(self, surface: pygame.Surface, blit_only: bool = True):
        game_surf = surface.subsurface(pygame.Rect(0, 0, self.WIDTH, self.HEIGHT)) if self.render_mode == "human" else surface
        ...
        if self.render_mode == "human" and hasattr(self, 'debug_manager'):
            try:
                ...
                if not self._debug_overlay_error_logged:
                    print(f"[Sonic Debug] overlay render error: {exc}")
                    self._debug_overlay_error_logged = True
