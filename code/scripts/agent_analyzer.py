...
def get_latest_log_file():
    """Return the most recently modified CSV in the csv/ folder (fallback: root/mylogs/runs)."""
    # Primary: dedicated csv/ folder
    primary = glob.glob(os.path.join("csv", "training_log*.csv"))
    if primary:
...
def analyze_agent():
    ...
    if len(all_files) == 1:
        ...
        print(f"📂 Found {len(all_files)} CSV log(s) in csv/ — combining all:")
        frames = []
        for f in all_files:
            print(f"   • {f}")
            try:
        ...
        print(f"   → {len(df):,} total rows combined\n")
