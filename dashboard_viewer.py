...
def format_arch_label(tag: str) -> str:
    labels = {
        "lightmobile": "LightMobile / Depthwise",
        "spatialattention": "SpatialAttention",
        "channelattention": "ChannelAttention",
        "deepchannelattention": "DeepChannelAttention",
        "mlp": "MlpPolicy",
    }
    return labels.get(str(tag or "").lower(), tag or "unknown")
...
def ensure_core_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Backfill core columns so the dashboard can open while a run is warming up."""
    out = df.copy()
    defaults = {
        "step": list(range(len(out))),
        "total_reward": 0.0,
        "action": "",
        "level": "",
        "levels_completed": 0,
...
def _level_progress_ratio(lvl_rows: pd.DataFrame) -> float:
    """Estimate farthest progress reached on a level, independent of win rate."""
    if lvl_rows.empty:
        return 0.0

    if "event" in lvl_rows.columns:
        win_mask = lvl_rows["event"].astype(str).eq("WIN")
        if win_mask.any():
            return 1.0

    estimates = []

    if "goal_dist" in lvl_rows.columns:
        goal_dist = pd.to_numeric(lvl_rows["goal_dist"], errors="coerce").dropna()
        if not goal_dist.empty:
            estimates.append(float((1.0 - goal_dist.clip(lower=0.0, upper=1.0)).max()))

    if not estimates and "progress" in lvl_rows.columns:
        progress = pd.to_numeric(lvl_rows["progress"], errors="coerce").fillna(0.0)
        if not progress.empty:
            cumulative = float(progress.clip(lower=0.0).sum())
            estimates.append(max(0.0, min(1.0, cumulative)))

    return max(0.0, min(1.0, max(estimates) if estimates else 0.0))


def _progress_bar(progress_ratio: float) -> str:
    pct = max(0.0, min(1.0, float(progress_ratio)))
    filled = int(round(pct * 10))
    bar = "\u2588" * filled + "\u2591" * (10 - filled)
    return f"{pct * 100:5.1f}%  {bar}"


def _build_level_rows(data_so_far: pd.DataFrame) -> list[dict]:
    if "level" not in data_so_far.columns or "event" not in data_so_far.columns:
        return []

    seen_order = data_so_far["level"].dropna().astype(str)
    seen_order = [lvl for lvl in seen_order.unique().tolist() if lvl.strip()]
    rows_list = []
    for lvl in seen_order:
        lvl_rows = data_so_far[data_so_far["level"] == lvl]
        event_series = lvl_rows["event"].astype(str).str.strip().str.upper()
        cause_series = lvl_rows.get("cause", pd.Series(index=lvl_rows.index, dtype=object)).astype(str).str.strip().str.title()
        died_rows = lvl_rows[event_series == "DIED"]
        died_causes = cause_series.loc[died_rows.index] if not died_rows.empty else pd.Series(dtype=object)
        wins = int((event_series == "WIN").sum())
        deaths = int((event_series == "DIED").sum())
        visits = wins + deaths
        win_rate = (wins / visits * 100.0) if visits > 0 else 0.0
        progress_ratio = _level_progress_ratio(lvl_rows)
        rows_list.append({
            "LEVEL": str(lvl),
            "VISITS": visits,
            "WINS": wins,
            "DEATHS": deaths,
            "STALL": int((died_causes == "Stall").sum()),
...
if len(all_logs) > 1:
    ...
    for f in all_logs:
        ...
        label = f"{info['persona']} | {info['skill']} | {format_arch_label(info['arch'])}  ({os.path.basename(f)})"
...
standard_cols = {
    'step', 'env_idx', 'total_reward', 'action', 'level', 'levels_completed',
    'x', 'y', 'vx', 'vy', 'goal_dist', 'progress', 'max_x_seen',
    'dijkstra_dist', 'step_dx', 'step_dy', 'first_completion_step',
    'event', 'cause',
}
_TELEMETRY_COLS = {
    'hp', 'lives', 'score',
    ...
    'enemies_killed_step', 'damage_taken_step',
    'life_lost', 'dijkstra_valid',
    'on_ladder', 'on_ground',
    'curriculum_level_idx', 'curriculum_win_rate', 'curriculum_max_unlocked',
}
_OBS_SANITY_COLS = {
...
for c in df.columns:
    ...
    if (
        lc in standard_cols
        or lc in _TELEMETRY_COLS
        or lc in _OBS_SANITY_COLS
        or "unnamed" in lc
    ):
        continue
...
col_i3.caption(f"Arch: **{format_arch_label(run_info.get('arch', '?'))}** | Persona: **{run_info.get('persona', '?')}**")
...
summary_rows = _build_level_rows(df.iloc[:selected_idx+1])
summary_total_wins = sum(r["WINS"] for r in summary_rows)
summary_total_deaths = sum(r["DEATHS"] for r in summary_rows)
summary_levels_beaten = sum(1 for r in summary_rows if r["WINS"] > 0)
...
c4.metric("LEVELS BEATEN", summary_levels_beaten)
c5.metric("WINS", summary_total_wins)
c6.metric("DEATHS", summary_total_deaths)
st.divider()
        ...
        def _style_row(r):
            ...
            for c in r.index:
                if c=="WIN RATE": styles.append(f"color:{wr_color}")
                    ...
                    try:
                        prog = float(str(r[c]).split("%", 1)[0])
                    except Exception:
                        prog = 0.0
                    prog_color = "#27ae60" if prog >= 90 else ("#e67e22" if prog >= 50 else "#e74c3c")
                    styles.append(f"color:{prog_color}")
...
if reward_cols:
    ...
    for i,col_name in enumerate(reward_cols):
        val = pd.to_numeric(pd.Series([row.get(col_name, 0.0)]), errors="coerce").iloc[0]
        if pd.isna(val):
            val = 0.0
...
with col_graph:
    ...
    if not show_graph:
        ...
        available_signals = [sig for sig in (['total_reward'] + reward_cols) if sig in df.columns]
        ...
        if selected_lines and 'step' in df.columns:
            start_viz = 0 if show_full_history else max(0, selected_idx-5000)
            subset = df.iloc[start_viz:selected_idx+1][['step']+selected_lines].copy()
            subset['step'] = pd.to_numeric(subset['step'], errors='coerce')

            numeric_lines = []
            dropped_lines = []
            for sig_name in selected_lines:
                subset[sig_name] = pd.to_numeric(subset[sig_name], errors='coerce')
                if subset[sig_name].notna().any():
                    numeric_lines.append(sig_name)
                else:
                    dropped_lines.append(sig_name)

            subset = subset.dropna(subset=['step'])
            ...
            if numeric_lines:
                try:
                    fig = px.line(subset, x='step', y=numeric_lines,
                ...
                st.caption("Selected signals are not numeric yet.")

            if dropped_lines:
                friendly = ", ".join(sig.replace("_", " ").title() for sig in dropped_lines[:6])
                suffix = "..." if len(dropped_lines) > 6 else ""
                st.caption(f"Skipped non-numeric signals: {friendly}{suffix}")
        else:
            st.caption("No graphable signals yet.")
...
if obs_cols:
    ...
    obs_numeric_cols = [c for c in obs_cols if c != "obs_warnings"]
    obs_stats_missing = bool(obs_numeric_cols) and all(
        abs(float(row.get(c, 0.0))) < 1e-12 for c in obs_numeric_cols
    )
    if warn_val and isinstance(warn_val,str) and warn_val:
        for w in warn_val.split("|"): st.error(f"[WARN] {w}")
    elif obs_stats_missing:
        st.info("Observation telemetry missing in this log row.")
    else:
    ...
    if has_grid_data:
        ...
        for ch in grid_channels:
            ...
            status="OK"
            if obs_stats_missing: status="MISSING"
            elif isinstance(std_v,(int,float)) and std_v<1e-6 and ch == "solid": status="DEAD"
        ...
        s_status="OK"
        if obs_stats_missing: s_status="MISSING"
        elif isinstance(s_std,(int,float)) and s_std<1e-8: s_status="DEAD"
        ...
        obs_rows.append({"CHANNEL":"DIJK (SCALAR)",
            ...
            "STD":"-","MIN":"-","MAX":"-",
            "STATUS":"MISSING" if obs_stats_missing else ("OK" if (isinstance(dijk,(int,float)) and dijk>0) else "ZERO")})
        ...
        def _style_obs(r):
            ...
            for c in r.index:
                if c=="STATUS":
                    ...
                    elif status=="MISSING": styles.append("color:#8aa1c7")
